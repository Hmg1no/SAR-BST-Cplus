...
Swarm Tester::generateTree(int teamSize, TREETYPE type){
...
int main(){
    ...
    Random typeGen(0,4);
    {
        ...
        team.setType(AVL);
        cout << "\nDump after converting the tree from BST to SPLAY: " << "\n\n";
        team.dumpTree();
        cout << endl;

        teamSize = 2;
        roboType = DEFAULT_TYPE;
        for(int i=0;i<teamSize;i++){
            ID = idGen.getRandNum();
            roboType = static_cast<ROBOTTYPE>(typeGen.getRandNum());
            if (i == teamSize / 2) tempID = ID;//we store this ID for later use
        ...
        cout << "\nDump after converting the tree from AVL to SPLAY: " << "\n\n";
        team.dumpTree();
        cout << endl;
